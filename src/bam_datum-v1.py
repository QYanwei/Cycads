...
def get_ref_segment(fasta, chr, start, end):
    fa = pyfastx.Fasta(fasta)
    # aln_segment = ''
    if chr in fa:
        aln_segment = fa.fetch(chr, (start, end))
    else:
        aln_segment = None
        print('Chromosome/contig not found:' + chr)
    return aln_segment
def get_ref_segment(fasta, chr, start, end):
    fa = pyfastx.Fasta(fasta)
    aln_segment = ''
...
chr = 'NZ_CP033020.1'
aln_segment = get_ref_segment(fasta, chr, start, end)
print(aln_segment)
...
def softclip_hardclip_remove(cigarstring):
    cigar_list = []
...
def mapping_summary(bam):
    ...
    for read in readprofile.fetch():
        ...
        if read.is_forward:
            isf += 1
        total_count += 1
        softclip_hardclip_remove(read.cigarstring)
